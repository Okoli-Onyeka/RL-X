...
class PPO:
    ...
    def get_inference_action(self, state, env_id_0, env_id_1):
        state_b = state[env_id_0:env_id_1]
        ...
        action_mean_b, action_logstd_b = self.policy.apply(self.policy_state.params, dynamic_joint_description_b, dynamic_joint_state_b, dynamic_foot_description_b, dynamic_foot_state_b, state_b[:, self.policy_general_state_mask[env_id_0]])

        action = jnp.concatenate([action_mean_b, jnp.zeros((self.nr_envs_per_env_type, self.missing_nr_of_actions[env_id_0]))], axis=1)
        processed_action = self.get_processed_action(action)

        return jax.device.get(processed_action)


    def get_interference_action_multi_render(self, state, env_id):
        dynamic_joint_combined_state_b = state[:, :self.dynamic_joint_observation_lengths[env_id]].reshape((-1, self.nr_dynamic_joint_observations[env_id], self.single_dynamic_joint_observation_length))
        dynamic_joint_description_b = dynamic_joint_combined_state_b[:, :, :self.dynamic_joint_description_size]
        dynamic_joint_state_b = dynamic_joint_combined_state_b[:, :, self.dynamic_joint_description_size:]
        ...
        action_mean_b, action_logstd_b = self.policy.apply(self.policy_state.params, dynamic_joint_description_b, dynamic_joint_state_b, dynamic_foot_description_b, dynamic_foot_state_b, state[:, self.policy_general_state_mask[env_id]])

        action = jnp.concatenate([action_mean_b, jnp.zeros((self.nr_envs_per_env_type, self.missing_nr_of_actions[env_id]))], axis=1)
        processed_action = self.get_processed_action(action)

        return jax.device_get(processed_action)
