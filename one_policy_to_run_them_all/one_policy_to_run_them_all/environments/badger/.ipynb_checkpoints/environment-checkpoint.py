...
from environments.multi_robot.residualtrackingreward import ResidualTrackingReward
...
class Badger(gym.Env):
    ...
    def __init__(self, seed, render,
        ...
        self.residual_reward_function = ResidualTrackingReward(self)
        ...
        self.residual_action = np.zeros(self.model.nu, dtype=np.float32)
        self.applied_residual_action = np.zeros(
            self.model.nu,
            dtype=np.float32
        )
        ...
        self.actual_yaw_displacement = 0.0
        self.initial_residual_observation = self.get_initial_residual_observation()
    ...
    def reset(self, seed=None):
        ...
        commands = self.make_commands(
            x_values=[1.0, 0.8, 0.6, 0.4, 0.2],
            y_values=[1.0, 0.8, 0.6, 0.4, 0.2],
            yaw_values=[1.0, 0.8, 0.6, 0.4, 0.2],
        )

        base_command = np.array(
            commands[self.np_rng.integers(len(commands))]
        )

        self.goal_x_velocity = base_command[0]
        self.goal_y_velocity = base_command[1]
        self.goal_yaw_velocity = base_command[2]

        self.residual_action = np.zeros(
            self.model.nu,
            dtype=np.float32
        )
        ...
        self.residual_reward_function.setup()
    ...
    def step(self, action):
        base_action = action[:self.model.nu]
        ...
        self.applied_residual_action = self.residual_scale * residual_action
        combined_action = base_action + self.applied_residual_action

        action = self.domain_randomization_action_delay_function.delay_action(combined_action)
        ...
        #get residual values
        residual_reward, info = self.residual_reward_function.reward_and_info(info, done)
        info["residual_reward"] = residual_reward

        self.residual_reward_function.step(residual_action)
        ...
        self.actual_yaw_displacement += (float(self.data.qvel[5]) * self.dt)
    ...
    def get_initial_residual_observation(self):
        ...
        return np.concatenate([
            ...
            goal_velocity,
            self.current_action,
            self.applied_residual_action,
        ]).astype(np.float32)

    def get_residual_observation(self):

        observation = self.initial_residual_observation.copy()
        observation[0:3] = self.orientation_quat_inv.apply(self.data.qvel[:3])
        observation[3:6] = self.data.qvel[3:6]
        observation[6:9] = np.array([self.goal_x_velocity, self.goal_y_velocity, self.goal_yaw_velocity])
        observation[9:22] = self.current_action
        observation[22:35] = self.applied_residual_action

        return observation
    
    def set_residual_action(self, residual_action):
        self.residual_action = residual_action

    def get_residual_action(self):
        return self.residual_action
    
    def get_trunk_position(self) -> float:
        current_trunk_x = float(
            self.data.body("trunk").xpos[0]
        )

        current_trunk_y = float(
            self.data.body("trunk").xpos[1]
        )

        return current_trunk_x, current_trunk_y
    
    def get_displacement(self) -> float:
        current_trunk_x, current_trunk_y = self.get_trunk_position()

        actual_x_displacement = (
            current_trunk_x - self.initial_trunk_x
        )

        actual_y_displacement = (
            current_trunk_y - self.initial_trunk_y
        )

        actual_yaw_displacement = (
            self.actual_yaw_displacement
        )

        return actual_x_displacement, actual_y_displacement, actual_yaw_displacement
    
    def get_displacement_tracking(self, episode_step) -> dict:
        """
        Return actual displacement, desired displacement and tracking error.
        """
        elapsed_time = episode_step * self.dt

        actual_x_displacement, actual_y_displacement, actual_yaw_displacement = self.get_displacement()

        desired_x_displacement = (
            self.goal_x_velocity * elapsed_time
        )

        desired_y_displacement = (
            self.goal_y_velocity * elapsed_time
        )

        desired_yaw_displacement = (
            self.goal_yaw_velocity * elapsed_time
        )

        x_error = desired_x_displacement - actual_x_displacement
        y_error = desired_y_displacement - actual_y_displacement
        yaw_error = desired_yaw_displacement - actual_yaw_displacement

        return {
        "time": float(elapsed_time),

        "desired_x_displacement": float(desired_x_displacement),

        "actual_x_displacement": float(actual_x_displacement),

        "x_displacement_error": float(x_error),

        "absolute_x_displacement_error": float(abs(x_error)),

        "desired_y_displacement": float(desired_y_displacement),

        "actual_y_displacement": float(actual_y_displacement),

        "y_displacement_error": float(y_error),

        "absolute_y_displacement_error": float(abs(y_error)),

        "desired_yaw_displacement": float(desired_yaw_displacement),

        "actual_yaw_displacement": float(actual_yaw_displacement),

        "yaw_displacement_error": float(yaw_error),

        "absolute_yaw_displacement_error": float(abs(yaw_error)),
    }

    def make_commands(self, x_values, y_values, yaw_values, include_zero=True):
        commands = []

        for value in x_values:
            commands += [(value, 0.0, 0.0), (-value, 0.0, 0.0)]

        for value in y_values:
            commands += [(0.0, value, 0.0), (0.0, -value, 0.0)]

        for value in yaw_values:
            commands += [(0.0, 0.0, value), (0.0, 0.0, -value)]
